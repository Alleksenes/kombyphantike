...
def infer_difficulty_scores():
    if not CSV_PATH.exists():
        ...
        try:
            ...
            if lemma_col not in df.columns:
                logging.warning(
                    f"Could not find Lemma column in {CSV_PATH}. Columns: {df.columns}"
                )
                kelly_map = {}
            else:
    ...
    if inheritance_candidates:
        logging.info(
            f"Found {len(inheritance_candidates)} candidates for KDS inheritance."
        )
        ...
        cursor.executemany(
            "UPDATE lemmas SET kds_score = ? WHERE id = ?", inheritance_updates
        )
        conn.commit()
    else:
    ...
    conn.close()
    logging.info(
        f"Done. Initial update: {len(updates)} lemmas. Inheritance update: {len(inheritance_candidates) if inheritance_candidates else 0} lemmas."
    )
