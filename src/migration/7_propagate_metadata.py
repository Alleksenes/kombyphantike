...
def propagate_metadata(db_path=DB_PATH):
    ...
    Fields propagated: greek_def, english_def, etymology_text, lsj_id, shift_type.
    ...
    SELECT
        child.id,
        child.greek_def, child.english_def, child.etymology_text, child.lsj_id, child.shift_type,
        parent.greek_def, parent.english_def, parent.etymology_text, parent.lsj_id, parent.shift_type
    ...
    for row in rows:
        (c_id, c_greek, c_eng, c_etym, c_lsj, c_shift,
         p_greek, p_eng, p_etym, p_lsj, p_shift) = row
        ...
        if (new_greek != c_greek or
            ...
            cursor.execute("""
                UPDATE lemmas
                SET greek_def = ?, english_def = ?, etymology_text = ?, lsj_id = ?, shift_type = ?
                WHERE id = ?
